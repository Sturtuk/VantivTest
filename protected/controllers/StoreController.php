	...
	{				
		...
		if (is_array($res) && count($res)>=1){
			...
			if ( $res['status']=="active" && $res['is_ready']==2){
				...
				/*-------------- Code for getting restaurant_name from id starts----------------------------------*/
				$store_merch_id = Yii::app()->session['var'];
				$cart_res_name = Yii::app()->db->createCommand()
						->select('restaurant_name,restaurant_slug')
						->from('mt_merchant')
						->where('merchant_id=:id', array(':id'=>$store_merch_id)) 
						->queryAll();
				//print_r($cart_res_name);
				/*-------------- Code for getting restaurant_name from id ends----------------------------------*/
				$this->render('menu' ,array(
				   'cart_res_name'=>$cart_res_name,
				   'data'=>$res,
	...
	public function actionBrowse()
	{
		...
		switch ($_GET['tab']){			
		    ...
		    case 3:
		      ...
		      case 4:
			  $tabs=4;
			  $list=Yii::app()->functions->featuredMerch();	      
		      break;  
